...
function parseJobs(raw){
  const a=raw.indexOf('export const liveJobs =');
  const s=raw.indexOf('[',a);
  const m=raw.indexOf('export const discoveryMeta',s);
...
console.log('secondaryHigh:',secondaryHigh.length,'suspiciousHigh:',suspiciousHigh.length,'longOverseas:',longOverseas.length);

function line(x){
  const j=x.job,r=x.result;
  return `${r.level}\tP${r.priorityScore}\tFit${r.fit.score}\tQ${r.dataQuality.score}/${r.dataQuality.status}\tRisk-${r.risk.deduction}\t${j.company}\t${j.title}\t${j.city||'-'}\t${r.direction}\t${j.sourceType||'-'}\t${r.gate.reasons.join('|')||'-'}`;
}
console.log('\n--- TOP 30 ---');
...
const report={
  ...
  secondaryHigh:secondaryHigh.length, suspiciousHigh:suspiciousHigh.length,
...
// Guardrails: calibration should fail only for structural regressions, not distribution preferences.
