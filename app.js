...
// 本地时区今天的 YYYY-MM-DD
function todayStr(){const n=new Date();const p=(x)=>String(x).padStart(2,"0");return n.getFullYear()+"-"+p(n.getMonth()+1)+"-"+p(n.getDate());}
// 今日新增 = 该公司今天录入公司库（companyMeta.addedAt === 今天）
function isNewToday(v){return !!(v&&companyMetaFor(v)&&companyMetaFor(v).addedAt===todayStr());}
// 公司是否今日新增
function isNewCompanyToday(name){return !!(name&&companyMetaFor(name)&&companyMetaFor(name).addedAt===todayStr());}
...
function filtered(){
  ...
  return pool.filter(j=>{
    ...
    return(!q||hay.includes(q))
      ...
      &&(!f.newToday||isNewCompanyToday(j.company));
  });
}
  ...
  return`<article class="row">
    ...
    <div class="cell-main">
      ...
      <div class="tags">
        ...
        ${isNewCompanyToday(j.company)?`<span class="tag new" title="该公司今日新录入公司库">今日新增</span>`:''}
    ...
    <header class="co-head">
      ...
      <div class="co-id">
        ...
        <div class="co-meta">
          ...
          ${isNewCompanyToday(g.name)?`<span class="tag new">今日新增</span>`:''}
          ${recPills}
          ${highRisk?`<span class="tag bad">历史风险 A/B·${highRisk}</span>`:''}
          ${internRisk?`<span class="tag warn">实习留用线索 ${internRisk}</span>`:''}
        </div>
...
function jobsPage(){
  ...
  <section class="hero"><div class="hero-in">
    ...
    ${(()=>{const newJobs=normal.filter(j=>isNewCompanyToday(j.company));const newCos=Object.keys(S.companyMeta||{}).filter(k=>S.companyMeta[k].addedAt===todayStr());return (newCos.length||newJobs.length)?`<div class="today-banner"><span class="today-badge">今日新增</span><span>公司 <b>${newCos.length}</b> 家 · 岗位 <b>${newJobs.length}</b> 个</span><button class="chip new ${S.filter.newToday?'active':''}" data-qnewtoday="1">只看今日新增</button></div>`:'';})()}
