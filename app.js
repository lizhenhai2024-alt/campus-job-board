...
function filtered(){
  ...
  return pool.filter(j=>{
    ...
    return(!q||hay.includes(q))&&(f.level==='全部'||ev.level===f.level||(f.level==='S'&&ev.level==='S++'))&&(f.direction==='全部'||ev.direction===f.direction)&&(f.city==='全部'||j.city===f.city)&&(f.source==='全部'||src===f.source)&&(f.quality==='全部'||ev.dataQuality.status===f.quality);
  });
}
