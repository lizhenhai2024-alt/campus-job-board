...
(function(root){
  ...
  function englishMajorCompatible(text=''){
    return /英语专业|英语类|外语类|外语专业|外国语言文学|语言类|翻译类|英语、|英语\/|英语或/.test(String(text||''));
  }
  ...
  function majorLanguageScore(job){
    ...
    else if(englishMajorCompatible(t) || englishMajorCompatible(ev)) score=20;
    else if(MARKET_BUSINESS.test(ev||t)) score=16;
    if(STEM.test(ev) && !MARKET_BUSINESS.test(ev) && !englishMajorCompatible(t) && !englishMajorCompatible(ev)) score=8;
    if(/理工.*优先|工科.*优先|技术背景优先|计算机.*优先/.test(t) && !englishMajorCompatible(t) && !englishMajorCompatible(ev)) score=Math.min(score,10);
